...
def main(args: Namespace) -> None:
    ...
    # test model
    if args.train==0:
        model.load_state_dict(torch.load(str(args.save_dir)+'/'+args.model+'.pt'))
...
def build_model(args) -> nn.Module:
    if args.model == 'LSTM':
        ...
        model = TCN(2,2,[16,16,16]).to(args.device)
    return model
...
def train(args: Namespace, model: nn.Module, train_loader: DataLoader, model_name: str) -> None:
    ...
    torch.save(model.state_dict(), str(args.save_dir) + model_name + '.pt')
...
def parse_args() -> Namespace:
    ...
    parser.add_argument(
        "--save_dir",
        type=Path,
        help="Directory to the saved model.",
        default="./models/",
    )
